...
import com.example.gavno.databinding.ActivityMapBinding
...
class MapActivity : AppCompatActivity(), CameraListener {
        ...
        if (finished) {
            when {
                cameraPosition.zoom >= ZOOM_BOUNDARY && zoomValue <= ZOOM_BOUNDARY -> {
                    placemarkMapObject.setIcon(ImageProvider.fromBitmap(createBitmapFromVector(R.drawable.metrablue)))
                }

                cameraPosition.zoom <= ZOOM_BOUNDARY && zoomValue >= ZOOM_BOUNDARY -> {
                    placemarkMapObject.setIcon(ImageProvider.fromBitmap(createBitmapFromVector(R.drawable.metrared)))
                }
            }
    ...
    private fun setMarker(location: Point) {
        ...
        placemarkMapObject = mapObjectCollection.addPlacemark(
            location,
            ImageProvider.fromBitmap(createBitmapFromVector(R.drawable.metka)!!)
        )
    ...
    private fun setMarkerInStartLocation() {
        val marker = createBitmapFromVector(R.drawable.metka)
